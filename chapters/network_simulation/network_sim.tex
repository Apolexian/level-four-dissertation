\chapter{Network simulation} \label{chap:net_sim}
...
The script takes several parameters to create the following scenarios:
...
However, we found it difficult to find exact data on packet loss percentages in smart factories or workshops; hence, we had to 
estimate these.
...
Therefore, giving an error margin on these percentages is also difficult.
This uncertainty is why we chose to present a synthetic scenario with extreme packet loss as presented in Table~\ref{tab:links:synth}.
...
We evaluated MQuicTT's performance using the presented topologies and respective simulation parameters.
